...
// Shared Barnes-Hut octree (monopole aggregates) used by both the
// Newtonian tree gravity and the field-based 1PN force model. Internal
// implementation detail: the node pool is a flat vector and child links
// are indices; never hold a Node reference across nodes.push_back (the
...
inline int octant_index(const Node& node, const Vec3& position) noexcept {
    int index = 0;
...
inline Node child_cell(const Node& parent, int octant) {
    Node child;
...
// Insert body_index into the subtree rooted at node_index.
inline void insert_particle(
